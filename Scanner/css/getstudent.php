...
//$result = mysql_query( "select series_id from reservation_instances where DATE(start_date) = CURDATE()"); 
  ...
  $result = mysql_query( "select series_id from reservation_instances WHERE (TIME( NOW( ) ) 
BETWEEN DATE_ADD(TIME( start_date ),INTERVAL 2 HOUR)
AND DATE_ADD(TIME( end_date ), INTERVAL 2 HOUR)  and (DATE(start_date) = CURDATE()) ");     
...
//opvragen examentijd  
$result5 = mysql_query("SELECT start_date,end_date from reservation_instances where series_id = '$sid'" );
$row5= mysql_fetch_row($result5);  
$timeStart = date("H:i",strtotime($row5[0]));
$timeStop = date("H:i",strtotime($row5[1]));
...
echo "groep: " . $groepnaam;
    ...
    echo " Geen rijen gevonden,probeer het later nog een keer!";
    exit;
}
